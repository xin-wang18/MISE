...
from src.utils.dataset_utils import NERDataset, MAMLDataset
...
from src.utils.maml import Learner
...
def train_base(opt, train_examples, dev_examples=None):
    ...
    train_features, train_callback_info = convert_examples_to_features(opt.task_type, train_examples,
                                                  opt.max_seq_len, opt.bert_dir, ent2id)
    # print(train_features[-1].__dict__)
    # print(train_callback_info[0][-1])
    # exit()
    # train_dataset = NERDataset(opt.task_type, train_features, 'train', use_type_embed=opt.use_type_embed)
    train_dataset = MAMLDataset(opt, train_features, train_callback_info,'train', use_type_embed=opt.use_type_embed)
    ...
    if dev_examples is not None:
        dev_features, dev_callback_info = convert_examples_to_features(opt.task_type, dev_examples,
                                                                       opt.max_seq_len, opt.bert_dir, ent2id)
        # print(len(dev_callback_info[0]))
        # print(dev_callback_info[0])
        # exit()
        dev_dataset = MAMLDataset(opt, dev_features, dev_callback_info, 'dev', use_type_embed=opt.use_type_embed)
...
def training(opt):
    ...
    train_raw_examples = processor.read_json(os.path.join(opt.raw_data_dir, 'stressor_train.json'))
    # all_raw_examples = processor.read_json(os.path.join(opt.raw_data_dir, 'all.json'))
    # print(len(all_raw_examples))
    # exit()
    # add pseudo data to train data
    # pseudo_raw_examples = processor.read_json(os.path.join(opt.raw_data_dir, 'pseudo.json'))
    # print(train_raw_examples)
    # print(len(train_raw_examples))
    # exit()
    ...
    # exit()
    # print(train_examples[0].pseudo,train_examples[0].distant_labels)
    dev_examples = None
    if opt.eval_model:
        dev_raw_examples = processor.read_json(os.path.join(opt.raw_data_dir, 'stressor_test.json'))
        dev_examples = processor.get_examples(dev_raw_examples, 'train')
...
#     stack_raw_examples = processor.read_json(os.path.join(opt.raw_data_dir, 'all.json'))
...
#         # print(len(train_ids),len(dev_ids))
#         # exit()
...
if __name__ == '__main__':
    ...
    args.output_dir += f'_alpha_{args.alpha}'
    args.output_dir += f'_T_{args.T}'
    args.output_dir += f'_shot_{args.shot}'
    args.output_dir += f'_query_{args.query}'
    args.output_dir += f'_metalr_{args.meta_lr}'
    args.output_dir += f'_innerlr_{args.update_lr}'
