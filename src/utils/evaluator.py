...
def get_base_out(model, loader, device):
    ...
    # with torch.no_grad():
    for step, (x_spt, x_qry) in enumerate(loader):
    # for idx, _batch in enumerate(loader):
        # print("the step is: ",step)
        x_spt = {key: x_spt[key].cuda(device) for key in x_spt}
        x_qry = {key: x_qry[key].cuda(device) for key in x_qry}
        # for key in _batch.keys():
        #     _batch[key] = _batch[key].to(device)

        tmp_out = model(x_spt,x_qry,training = False)
        callback_index = np.array({key: x_qry[key][:5] for key in x_qry}['pseudo'].cpu())
        callback_index = callback_index.ravel()
        # print("the result is",len(tmp_out))
        # exit()
        yield tmp_out, callback_index
# def get_base_out(model, loader, device):
#     """
#     每一个任务的 forward 都一样，封装起来
#     """
...
def crf_evaluation(model, dev_info, device, ent2id):
    ...
    # print(len(dev_callback_info),len(type_weight))
    # exit()
    pred_tokens = []
    dev_callback_index = []
    for tmp_pred,tmp_callback_index in get_base_out(model, dev_loader, device):
        # pred_tokens.extend(tmp_pred[0])
        pred_tokens.extend(tmp_pred)
        dev_callback_index.extend(tmp_callback_index)
    # print(pred_tokens)
    # print(dev_callback_index)
    # print(len(pred_tokens),len(dev_callback_index))
    # exit()
    dev_callback_info = np.array(dev_callback_info)[dev_callback_index].tolist()
    # print(dev_callback_info)
    # for a,b in zip(pred_tokens,dev_callback_info):
    #     print(a,b)
    # print(pred_tokens[0],dev_callback_info[0])
    # print(len(pred_tokens),len(dev_callback_info))
    # exit()
